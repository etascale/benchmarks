...
#ifndef __OMPSS_CLUSTER_CPP_BENCHMARKS_MEMORY_HPP__
#define __OMPSS_CLUSTER_CPP_BENCHMARKS_MEMORY_HPP__
...
static inline T* lmalloc(size_t size)
{
	...
	if (!alloc)
		std::cerr << "Could not allocate nanos6 "
			  << "local memory"
			  << std::endl;
	return alloc;
}
...
#endif /* __OMPSS_CLUSTER_CPP_BENCHMARKS_MEMORY_HPP__ */
